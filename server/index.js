...
const sessionStore = new pgSession({
    pool: db.pool,
    tableName: 'session',const express = require('express');
const cors = require('cors');
const cookieParser = require('cookie-parser');
const session = require('express-session');
const pgSession = require('connect-pg-simple')(session);
...
// Authentication Middleware - PROTEGIDO CONTRA ERRO 500 (VALIDAÇÃO DE SESSÃO)
app.use((req, res, next) => {
    ...
    // Verificação robusta: se req.session não existir ou não tiver userId, barra o acesso antes de quebrar o app
